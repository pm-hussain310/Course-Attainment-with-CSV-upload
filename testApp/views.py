...
def home(request):
    ...
    internals = Intenal_Question.objects.all()
    if request.method == 'POST':
        ...
        if Course_Details.objects.exists():
            Course_Details.objects.all().delete()
            Course_Details.objects.create(regulation=regulation, year=year, semester=semester,
                                            subject_name=subject_name,subject_code=subject_code,  academic_year=academic_year)
...
def attainment_levels(request):
    return render (request, 'testApp/attainment_levels.html')

def load_marks_with_csv(request):
    if request.method == 'POST':
        print()
    ...
    return render(request, 'testApp/load_marks_with_csv.html')

def indirect_attainment(request):
    if Indirect.objects.all().exists():
        # Indirect.objects.all().delete()
        cos = CO.objects.all()
        if request.method == 'POST':
            indirects = Indirect.objects.all()
            for i in indirects:
                indirect_id = request.POST.get("indirect_"+str(i.id))
                print(request.POST.get("high_"+str(i.id)))
                ...
                students_count = int(high) + int(medium) + int(low)
                attainment = round((high*3+medium*2+low*1)/students_count,2)
...
def interna_question(request):
    ...
    if request.method=='POST':
        ...
        print(co_ids)
        intenal_question = Intenal_Question.objects.create(question=question)
        intenal_question.save()
        for co_id in co_ids:
            co_mapped = CO.objects.get(id=co_id)
            intenal_question.co_mapped.add(co_mapped)
        messages.success(request, " Successfully saved the question")

    internals = Intenal_Question.objects.all()
    return render(request, 'testApp/interna_question.html', locals())

def edit_interna_question(request):
    cos = CO.objects.all()
    internals = Intenal_Question.objects.all()
    if request.method=='POST':
        for q in internals:
            question = request.POST.get('question'+str(q.id))
            intenal_question = Intenal_Question.objects.get(id=q.id)
            intenal_question.question = question
            intenal_question.save()
            q.co_mapped.clear()
            co_ids = request.POST.getlist('co_id'+str(q.id))
            for co_id in co_ids:
                co_mapped = CO.objects.get(id=co_id)
                intenal_question.co_mapped.add(co_mapped)
        return redirect('edit_external_question')
    return render(request, 'testApp/edit_interna_question.html', locals())

def external_question(request):
    cos = CO.objects.all()
    if request.method=='POST':
        qno = request.POST.get('qno')
        question = request.POST.get('question')
        co_ids = request.POST.getlist('co_id')
        print(co_ids)
        external_question = External_Question.objects.create(question=question)
        external_question.save()
        for co_id in co_ids:
            co_mapped = CO.objects.get(id=co_id)
            external_question.co_mapped.add(co_mapped)
        messages.success(request, " Successfully saved the question")
    external = External_Question.objects.all()
    print(external)
    return render(request, 'testApp/external_question.html', locals())

def edit_external_question(request):
    cos = CO.objects.all()
    external = External_Question.objects.all()
    if request.method=='POST':
        for q in external:
            question = request.POST.get('question'+str(q.id))
            external_question = External_Question.objects.get(id=q.id)
            external_question.question = question
            external_question.save()
            q.co_mapped.clear()
            co_ids = request.POST.getlist('co_id'+str(q.id))
            for co_id in co_ids:
                co_mapped = CO.objects.get(id=co_id)
                external_question.co_mapped.add(co_mapped)
        return redirect('co_po_matrix1')
    return render(request, 'testApp/edit_external_question.html', locals())

def course_attainment(request):
    cos = CO.objects.all()
    Course_Attainment.objects.all().delete()
    for co in cos:
        # print(Intenal_Question.objects.filter(co_mapped=co).aggregate(Avg('attainment_level')))
...
def co_po_matrix1(request):
    if CO_PO_Matrix.objects.exists():
        ...
        if request.method == 'POST':
            ...
            return redirect('course_attainment')
...
def home1(request):
    ...
    ext_gth_count = StudentMark.objects.filter(ext_marks__gte=0.40*75).count()
    import math
    m1_q1_gth_percnt = round((m1_q1_gth_count/m1_q1_attempt_count)*100) if m1_q1_attempt_count is not None or 0 else None
    m1_q2_gth_percnt = round((m1_q2_gth_count/m1_q2_attempt_count)*100) if m1_q2_attempt_count is not None or 0 else None
    m1_q3_gth_percnt = round((m1_q3_gth_count/m1_q3_attempt_count)*100) if m1_q3_attempt_count is not None or 0 else None
    m1_q4_gth_percnt = round((m1_q4_gth_count/m1_q4_attempt_count)*100) if m1_q4_attempt_count is not 0 or None else None
    m1_quiz_gth_percnt = round((m1_quiz_gth_count/m1_quiz_attempt_count)*100) if m1_quiz_attempt_count is not None or 0 else None
    m1_assign_gth_percnt = round((m1_assign_gth_count/m1_assign_attempt_count)*100) if m1_assign_attempt_count is not None or 0 else None
    m2_q1_gth_percnt = round((m2_q1_gth_count/m2_q1_attempt_count)*100) if m2_q1_attempt_count is not 0 or None else None
    m2_q2_gth_percnt = round((m2_q2_gth_count/m2_q2_attempt_count)*100) if m2_q2_attempt_count is not None or 0 else None
    m2_q3_gth_percnt = round((m2_q3_gth_count/m2_q3_attempt_count)*100) if m2_q3_attempt_count is not None or 0 else None
    m2_q4_gth_percnt = round((m2_q4_gth_count/m2_q4_attempt_count)*100) if m2_q4_attempt_count is not None or 0 else None
    m2_quiz_gth_percnt = round((m2_quiz_gth_count/m2_quiz_attempt_count)*100) if m2_quiz_attempt_count is not None or 0 else None
    m2_assign_gth_percnt = round((m2_assign_gth_count/m2_assign_attempt_count)*100) if m2_assign_attempt_count is not None or 0 else None
    ext_gth_percnt = round((ext_gth_count/ext_attempt_count)*100)

    # attainment levels
    print(Intenal_Question.objects.all().values())
    print(External_Question.objects.all().values())
    for i in Intenal_Question.objects.all():
        ...
        gth_percnt = round((gth_count/attempt_count)*100) if attempt_count is not 0 or None else 0
        if gth_percnt >=76:
    ...
    for i in External_Question.objects.all():
        attempt_count = StudentMark.objects.filter(ext_marks__gte=0).count()
        gth_count = StudentMark.objects.filter(ext_marks__gte=0.40*75).count()
    ...
    po11_attainment = (po11_avg*course_attainment_value)/3 if po11_avg is not None else None
    ...
    pso3_attainment = (pso3_avg*course_attainment_value)/3 if pso3_avg is not None else None


    #for Graph
    attained_values = []
    for co in CO.objects.all():
        attained_value = Course_Attainment.objects.get(co=co)
        attained_values.append(round(attained_value.final,2))
    print("attained_values : ", attained_values)
